...
function convertibleRows () {
  return [...state.rows.values()].filter(row => row.status === 'pending' || row.status === 'error')
}
...
function render () {
  listEl.replaceChildren(...[...state.rows.values()].map(row => {
    ...
    chip.textContent = STATUS_LABELS[row.status]
    item.append(chip)
  ...
  buttons.convert.textContent = state.running ? 'Converting…' : 'Convert all'
  ...
  outdirLabel.textContent = state.outputDir ? `Output: ${state.outputDir}` : 'Output: same folder as each file'
...
buttons.convert.addEventListener('click', async () => {
  ...
  try {
    ...
    showBanner(`Unexpected error: ${err.message}`)
  } finally {
    state.running = false
    render()
  }
